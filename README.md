# Solutions:
1. cd into the folder where you find file `manage.py`
2. create a virtualenv. The command I used on mac is
    `virtualenv -p python3 venv`
3. To run activate virtualenv run command 
    `source venv/bin/activate`
4. Use any of the following command to run the etl script:
    1. To run the script, please run the following command
    ```python etl_script/main.py```
    2. To run it the django way, use command 
    ```python manage.py runscript etl_script.main -v3```
5. to run server use command `python manage.py runserver`
6. Go to `http://127.0.0.1:8000/api/dashboard/` to see the django rest view
